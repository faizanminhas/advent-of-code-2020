...
function isValid(rule, password) {
  const [atLeast, atMost, letter] = rule.split(/-| /);
  const num = password.replace(new RegExp(`[^${letter}]`, 'gi'), '').length;
  return num >= atLeast && num <= atMost;
}

function isValid2(rule, password) {
  const [pos1, pos2, letter] = rule.split(/-| /);

  return Boolean(password[pos1] === letter ^ password[pos2] === letter);
}
...
for(let [rule, password] of inputs) {
  if (isValid2(rule, password)) {
    sum = sum + 1;
  }
}
