...
const requiredFields = {
  'byr': x => x.toString().length === 4 && within(1920, 2002, x),
  'iyr': x => x.toString().length === 4 && within(2010, 2020, x),
  'eyr': x => x.toString().length === 4 && within(2020, 2030, x),
  'hgt': x => (x.endsWith('cm') && within(150, 193, x.slice(0, -2))) ||
              (x.endsWith('in') && within(59, 76, x.slice(0, -2))),
  'hcl': x => x.startsWith('#') && x.substring(1).length === 6 && /^[0-9|a-f]*$/.test(x.substring(1)),
  'ecl': x => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(x),
  'pid': x => x.length === 9 && /^[0-9]*$/.test(x)
}
//, 'cid'];

function passportToObject(passport) {
  return Object.fromEntries(passport.join(' ').split(' ').filter(Boolean).map(x => x.split(':')));
}


function passportValid(passport) {
  const presentFields = passportToObject(passport);

  return Object.keys(requiredFields).every(field => field in presentFields && requiredFields[field](presentFields[field]));
}

console.log(parsedInput.reduce((total, passport) => total + (passportValid(passport) ? 1 : 0), 0))
